 ...
 */
import { fetchPlate, paymentUrl } from "../src/police.js";
...
for (const plate of plates) {
  ...
  for (const fine of fines) {
    const text = renderEvent({ type: "new", protocolNo: fine.protocolNo, fine }, plate);
    console.log("\n" + text.replace(/<[^>]+>/g, ""));
    console.log("💳 " + paymentUrl(fine.protocolNo, plate));
  }
}
